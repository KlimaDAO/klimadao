...
function title(params: BridgePageParams) {
  ...
  return t`${bridgeLabel} volume deposited over time`;
}
function description(params: BridgePageParams) {
  const bridgeLabel = getBridgeLabel(params.bridge);
  return t`The volume of deposited digital carbon credits in ${bridgeLabel} digital carbon pools over a given time period.`;
}
