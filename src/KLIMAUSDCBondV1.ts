...
export function handleDeposit(event: BondCreated): void {
    ...
    deposit.discount = getKLIMAUSDRate().div(deposit.bondPrice).minus(BigDecimal.fromString('1')).times(BigDecimal.fromString('100'))
...
export function handleRedeem(event: BondRedeemed): void {
    ...
    redemption.token = loadOrCreateToken(KLIMAUSDC_LPBOND_TOKEN).id;
