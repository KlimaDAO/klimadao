...
import { BCTUSDC_LPBOND_TOKEN } from './utils/Constants'
...
export function handleDeposit(event: BondCreated): void {
    ...
    let token = loadOrCreateToken(BCTUSDC_LPBOND_TOKEN)
...
export function handleRedeem(event: BondRedeemed): void {
    ...
    redemption.token = loadOrCreateToken(BCTUSDC_LPBOND_TOKEN).id;
